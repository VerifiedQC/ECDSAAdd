...
成本压缩按 [重做设计](docs/REWORK_PLAN.md) 分七项推进；目标数是按文档门列推导的预期值，以实现后的 Lean 资源定理为准。

| 项 | 内容 | 受控原地点加 Toffoli 目标 | 线路目标 | 负责 |
| --- | --- | ---: | ---: | --- |
| 改 1 | 求逆第二阶段改为在输出寄存器上原地模减半、只做一次 | ≈ 56M | ≈ 72k | Deutsch |
| 改 2 | 模乘改为 Horner 原地累加，不存倍数链（空间 O(n)） | ≈ 28M | ≈ 10k | Deutsch |
| 改 3 | 点加改为除法中心的原地更新（2 次除法 + 3 次模乘），角落情形仍由标志覆盖 | ≈ 15M | ≈ 5k | Deutsch |
| 改 4 | Gidney 比较器（n Toffoli）替代两次减法的借位比较，接口陈述不变 | ≈ 13.8M | ≈ 5k | Deutsch |
| 改 5 | Kaliski 轮压缩（MBU 零检测、原地受控加减） | ≈ 13M | ≈ 5k | 待安排 |
| 改 6 | Montgomery 4 位窗口模乘（6a 标准形式 / 6b 全 Montgomery 表示） | ≈ 11M / 10M | ≈ 5k | 待安排 |
| 改 7 | CCZ 修正，仅当改 6 采用测量反查表时需要 | — | — | 条件项 |

每项先提交设计 PR 描述（构造、逐步寄存器表、门数推导、证明义务、文件改动），复审确认后再写证明；公开定理陈述保持不变，只替换实现与资源数。

现 74,024 根线路中 69,908 是模乘倍数链与模加减工作区构成的共享池，改 2 后该池缩为求逆工作区。5k 以下的进一步削减（Kaliski 记录带每轮 1 位、常数加法器不用临时寄存器、比较/零检测链复用加法器辅助位）尚未纳入编号计划。
