...
| 改 3（设计中） | 除法中心原地更新：2 次除法内含 2 个乘积，另加 3 个乘积；输出侧标志清理 | 14,998,618（待证明） | 6,218 实际支持（待证明） | Deutsch |
...
改 3 的具体门列与寄存器表见 [§16 实施设计](docs/REWORK_PLAN.md#16-改-3-实施设计除法中心的受控原地点加待复审待实现)：以改5求逆与C2/D乘积接口推导，目标为14,998,618 Toffoli / 7,880,538测量 / 6,218实际支持线，均待复审与实现。保留公共受控原地点加规格；D的兼容布局仍分配9,817位，未将未触及位计入实际支持。当前已证值不变。
