...
本节描述当前提交包含的代码。M1（语义、Hoare 规格、AND 测量反计算）、M2（加减法、模 p 加减、模乘）、完整 EEA 求逆（I1–I5）和 M3（候选计算、完整经典常量点加、受控原地点加）已完成；改 1 已将求逆第二阶段替换为原地减半与显式加倍恢复；改 5 已用测量清零检测链并接入原地受控加减；改 4 已将计数活动比较及记录段接入 Gidney 比较器；改 2 已用 Horner 内核及三个适配器替换倍数链模乘，并缩小共享工作池。最终结果 `controlledPointAdd` 对任意合法点 R、经典常量 C 与控制位 b 证明 `point = if b then R+C else R`，清零全部工作位并对所有测量记录恢复相位；有限 C 的同程序精确资源为 32,347,957 个 Toffoli、17,585,440 次测量、9,718 根实际静态线路。改 1 已实现，后续门数压缩见下文计划。
...
M3 受控原地 `controlledPointAdd` 对有限 C 使用 **32,347,957 个 Toffoli、17,585,440 次测量、9,718 根实际静态线路**。三个选择位只控制最终输出，算术和测量程序不加控制；两次前向受控 XOR 调用加 513 位受控交换清空临时点。C=O 在构造期为空程序，三项资源均为零。实际支持比完整点加增加控制位与三个选择位，空间为 O(n+N)。
...
| 改 6 | Montgomery 4 位窗口模乘（6a 标准形式 / 6b 全 Montgomery 表示；研究预算） | ≈ 12M / 11M | ≈ 5k | Lamport（6a）；6b 待安排 |
| 改 7 | CCZ 修正，仅当改 6 采用测量反查表时需要 | — | — | 条件项 |

每项先提交设计 PR 描述（构造、逐步寄存器表、门数推导、证明义务、文件改动），复审确认后再写证明；公开定理陈述保持不变，只替换实现与资源数。

线路按实际门列支持计数。改 2 后共享池仍按求逆编号分配 5,699 位，模乘/模减/求逆的支持并集为 5,602 位；97 根旧 out 位没有门触及。受控点加的外部支持为 4,116 位，合计 9,718。更低空间的改 3 尚待实施。

改 2 的构造与资源见 [实施设计](docs/REWORK_PLAN.md#12-改-2-实施设计已实现)。C1/C2/D 已实现六个模算术原语、Horner 内核与三个适配器；XOR 模乘为 1,178,880 Toffoli / 916,736 测量 / 1,799 线，加/减适配器分别为 1,179,903/917,759 与 1,180,415/918,271，均为 1,799 线。集成保留 4 次求逆与 12 次模乘，按改 5 后实际程序重算为 32,347,957 Toffoli。受控半倍仍留待实际调用需求。
...
改 3 的具体门列与寄存器表见 [§16 实施设计](docs/REWORK_PLAN.md#16-改-3-实施设计除法中心的受控原地点加设计已复审电路待实现)：以改5求逆与C2/D乘积接口推导，目标为14,998,618 Toffoli / 7,880,538测量 / 6,218实际支持线，设计已复审，电路与资源目标仍待实现。保留公共受控原地点加规格；D的兼容布局仍分配9,817位，未将未触及位计入实际支持。当前已证值不变。

改3的八个数学引理已证明，见[证明状态](docs/PROOF_STATUS.md#改-3-数学原地更新与输出侧清理条件)：涵盖输出侧标志、普通分支域等式和第二除数为零时的例外斜率。除法已完成完整规格、逐线保持与精确资源，见[除法证明状态](docs/PROOF_STATUS.md#改-3-除法保留求逆历史的受控累加)：加5,722,415 Toffoli / 2,557,231测量，减5,722,927 / 2,557,743，均6,210根实际支持线。新的原地点加本体尚未接入，资源目标未替换当前已证值。
