...
改8受控加减的测量清掩码设计见 [REWORK_PLAN §20](docs/REWORK_PLAN.md#20-改-8montgomery-窗口受控加减的测量清掩码设计待复审与实现)。待证目标为每次782→521 Toffoli，同时测量260→521；只在Montgomery变量窗口替换。接改7的14/14查表后，完整点加目标11,001,338 Toffoli / 5,193,978测量 / 6,218线；当前已证值不变。

改8原语已实现：`MeasuredMaskedAdder.lean` 给出测量清掩码的受控原地加/减，完整Triple、目标外逐线保持与同程序资源为 `(2W−1, 2W−1, 4W+1)`；W=261时521 Toffoli /521测量 /1045线。尚未接入Montgomery窗口，完整点加仍为11,669,498/4,525,818/6,218。
