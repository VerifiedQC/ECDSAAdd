...
成本压缩按 [重做设计](docs/REWORK_PLAN.md) 分七项推进；目标数是按文档门列推导的预期值（标"研究预算"者未从已有门列推导），以实现后的 Lean 资源定理为准。依赖：先做基础层（原地加法器与原地模算术），改 1/2/4/5 只通过 Hoare triple 接口相互独立、可并行，改 3 依赖改 1 与改 2。

| 项 | 内容 | 受控原地点加 Toffoli 目标 | 线路目标 | 负责 |
| --- | --- | ---: | ---: | --- |
| 改 1 | 求逆第二阶段改为内部寄存器上的受控原地模减半与逆序加倍，XOR 接口不变 | ≈ 58M | 74,024（共享池由模乘决定） | Deutsch |
| 改 2 | 模乘改为 Horner 零输出内核 + 反序清理 + XOR/加/减适配器，不存倍数链（空间 O(n)） | ≈ 31M | ≈ 10k（与改 1 合计 ≈ 8k） | Deutsch |
| 改 3 | 点加改为除法中心的原地更新（2 次除法 + 5 个乘积），角落情形仍由标志覆盖 | ≈ 18M | ≈ 5k | Deutsch |
| 改 4 | Gidney 比较器（n Toffoli）替代两次减法的借位比较，接口陈述不变 | ≈ 16.5M | ≈ 5k | Deutsch |
| 改 5 | Kaliski 轮压缩（MBU 零检测、原地受控加减） | ≈ 15M | ≈ 5k | 待安排 |
| 改 6 | Montgomery 4 位窗口模乘（6a 标准形式 / 6b 全 Montgomery 表示；研究预算） | ≈ 13M / 12M | ≈ 5k | 待安排 |
