...
电路函数定义前的文档注释先概括输入输出变换，使用代码中的寄存器参数名，明确是 XOR 输出、原地更新还是将结果移入另一寄存器；随后只补充理解该变换必需的控制条件、模数/位宽、零工作区和保留输入。计算后仍保留的中间量与历史必须注明；恢复函数必须说明要求与哪一步的输出匹配，不能把任意未知数据说成可直接清零。具体布局和数值前提仍以 `_spec` / `_correct` 等定理为准，不在注释中重复证明。

定义注释还须有“参数”一段，按函数签名的顺序逐个说明参数的用途，不只复述 Lean 类型。wire 要标明输入、控制、输出或工作位；寄存器要区分保留输入、XOR 输出、原地目标和临时存储。对 `L`、`M` 等布局参数，说明本函数所用的输入、输出与工作字段；它们描述已有线路，不分配新 qubit。对模数、常量、轮号、窗口数、字段选择和子电路等构造期参数，明确其角色，避免与量子寄存器中保存的数值混淆。箭头式签名中的匿名参数按位置说明；别名继承的参数也要列出。证明中的定理参数和纯布局辅助定义不按电路函数批量展开。
