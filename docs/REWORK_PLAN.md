...
## 17. 改 6a：标准表示的四位窗口 Montgomery 设计（设计已复审，数学、lookup与P/Q已实现）
...
2. P/Q（已实现）：两个窗口循环、物理旋转、规范化/反规范化、显式历史契约、内部Triple及同程序资源/支持；五个输出适配器的Triple/资源为下一批。证明失败时修改设计，不放宽心跳/递归限制，不用sorry/native_decide/新公理或测试。
