...
每个完整适配器均包含两段Montgomery计算、两段恢复、标准表示转换、全部lookup清理、512位记录和两位借位清理；没有扣除无法复用的历史成本。三个值均低于600k条件目标，但现在不是已证资源。
...
3. 集成：保留fieldMul数值契约，与改3除法所有者核对共享工作区；重证实际支持及受影响资源，移除被替换的专用包装。旧Horner是否仍有调用者以实际引用决定，不能预先全删。
