...
| + 改 6a（Montgomery，标准形式） | 11,800,058（§17推导，待实现） | 待集成支持证明 | 2×4,541,488 + 2,703,515 + 13,567；包含两次受控适配器及平方步改写 |
...
| **受控原地点加（已证）** | **2I + 5P + 83n−6；测量2I_M+5M_P+70n−6** | **14,998,618** | **7,880,538** |

实际支持已证 **6,218**。相对 main 5560c9cf 的 52,914,997 Toffoli，推导减少 37,916,379；这包含 D 的每次乘积成本下降，不能全部归因于改3减少调用次数。D阶段基线为32,347,957；改3已证再减少17,349,339。D与改3的阶段资源分列。
...
## 17. 改 6a：标准表示的四位窗口 Montgomery 设计（设计已复审，数学与lookup已实现）

本节替代§7中未逐门展开的预算；只设计标准表示的模乘，不改点加算法或坐标表示。当前已证资源仍为改2的值。研究背景为[Häner 等，2020](https://arxiv.org/abs/2001.09580)的窗口算术与清理调度；以下具体门列、宽度与数字由本项目现有原语推导，不引用论文门数作为本实现证据。暂不采用15-Toffoli查表、约2n受控加法或CCZ修正。
...
再以 A 的四位窗口和经典常数 K=R² modp 执行同样迭代，得到 Z=AKR⁻¹ modp=XY modp。第二段使用查表 d↦dK 替代四次受控变量加法；K<p。两段最终约减各保留一位借位历史，直到清理阶段。本实现利用已证 p % 16 = 15：15·15 ≡ 1 (mod 16)，因此 −p⁻¹ ≡ 1 (mod 16)，修正系数 t=m，查表直接给 m·p，无需运行时模逆。K=R² mod p 的具体定义与转换电路留在后续 P/Q 批完成，不用外部数字证书或测试代替证明。
...
生产阶段P=`MontPrepare(X,Y,A); ConstPrepare(A,K,Z)`，清理阶段Q=`ConstRestore; MontRestore`。三个适配器统一使用 **P；输出更新；Q**：XOR用Z低257位复制，模加/减用C1普通原地模加/减。模加/减借table前257、carry前255、mask前257、cin和已归零的一个lookup scratch作为flag；Z低257位为源。
...
另外提供 `P；controlledModAdd c；Q` 与 `P；controlledModSub c；Q` 两条受控适配器，供§16两次除法使用。前提增加 `(c::L.wires).Nodup` 与 c=b；后置保持c与输入，out在b=true时加/减XY%p，否则保持O，work=0。P/Q不受控，b=false也完整计算并清理；相位仍覆盖全部测量记录。中段复用C1受控接口，借用同一mask/table/carry/cin及空lookup scratch作为flag，无新工作位。两者实际支持为普通适配器支持加控制位，目标2,597线。15-CCX单一迭代查表留作后续优化，当前预算不预支其收益。
...
每个完整适配器均包含两段Montgomery计算、两段恢复、标准表示转换、全部lookup清理、512位记录和两位借位清理；没有扣除无法复用的历史成本。五个值均低于600k条件目标，但现在不是已证资源。

工作区分配 `2W+512+2+2W+(W−1)+1+5+3 = 1,827`，外部770，合计2,597线；x最高位不使用，计划实际支持为其余**2,596线**。需逐门证明支持并集等式，再由Nodup得到基数；不能用分配数或同时存活数代替。pad、mask、carry等即使值恒零也由固定门列触及。暂不保证可原封不动嵌入改3除法借用区：需与Deutsch另核历史存活时的1,827工作位、输入/输出互异及支持，集成前不得将该乘法成本直接写成点加已证值。

### 17.6 实施与交付

1. 数学与lookup：约减整除/界/不变量/转换等式；lookup完整Triple/frame/计数。可单独复审。
2. P/Q：两个窗口循环、物理旋转、规范化/反规范化、显式历史契约，三适配器Triple及同程序资源/支持。证明失败时修改设计，不放宽心跳/递归限制，不用sorry/native_decide/新公理或测试。
3. 集成：保留fieldMul数值契约，**接入6a后§16.6的B分割与平方步按§17重写**：两次除法用受控模加/减适配器；外部模减、模加保持；平方改为一次mulSub(λ,S,x)，去掉原先单独的mulInto/mulClear与t字。五个适配器合计540,703+541,215+540,703+540,191+540,703=2,703,515 Toffoli；其余§16.7中段费用已包含在适配器内，剩13,567，故总预算2×4,541,488+2,703,515+13,567=11,800,058（待实现）。1,827工作位容量小于B的2,315位，具体映射与历史互异仍需证明。与改3除法所有者核对共享工作区；重证实际支持及受影响资源，移除被替换的专用包装。旧Horner是否仍有调用者以实际引用决定，不能预先全删。
...
§17第一批实现记录：Montgomery数学与lookup的Triple/frame/48+48计数已通过完整verify（2114构建、227公理入口）；支持给包含关系。P/Q、五个适配器及集成尚未实现，表中完整乘法和点加数字仍待证明。
