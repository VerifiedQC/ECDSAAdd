...
| 改 7 | 四位查表的测量清理（设计待复审，§18） | 反查表48 CCX/48测量 | 候选2 CCX/263测量；完整点加少88,320 CCX、多412,800测量 | 两个地址AND辅助位将修正降为Z/CZ，暂不需要CCZ扩展；全部待证明 |
...
当前四位地址、三根已清零lookup scratch允许另一具体构造：保留两对地址AND，把任意四位布尔函数写成地址和辅助位上的二次相位。§18列出完整门列、逐位即时修正与净账本；候选仅用已有Z/CZ，故不修改Syntax/Semantics/Cost或Triple规则。原先“k=4必须增加CCZ”的判断由此设计取代，是否实现以复审为准。当前已证程序仍使用重跑lookup清理。
...
| 改 7（四位查表测量清理） | 11,711,738（§18候选） | 6,218（待重证支持等式） | 以当前6a为基线，少88,320 CCX、多412,800测量；不含并行6b收益，待证明 |
...
## 18. 改 7：四位查表的测量清理（设计待复审，尚未实现）

### 18.1 目的、基线和公开契约

本节根据本仓库 `Lookup.lean`、`MontPrepare.lean`、`MontLookup.lean` 与 `Framework/Semantics.lean` 的字面门列推导，不将文献的查表渐近数当作实现成本。基线main为3d1e8bd：每次lookup48 CCX/48测量，`montLookupAdd/Sub`以lookup加载、算术更新acc、再次lookup清table。改7只替换**最后一次清理**，保留通用XOR查表的任意旧目标语义。
...
先交本设计复审；GO后实现。新增 `Arithmetic/LookupErase.lean`，内含私有四位系数/修正编译、相位恒等式、公开erase Triple/frame、计数和实际支持；不新建通用布尔合成框架。`MontPrepare.lean`只将montLookupAdd/Sub尾部改为erase，`MontLookup.lean`改组合证明；原 `lookup_spec` 完全保留。更新MontCounts/Resources及fieldMul、候选、除法、PointInPlace与最终资源常数和支持证明，不改变公开域/点加数值规格。
