...
## 18. 改 6b：全 Montgomery 表示与边界成本（设计，待复审）
...
这里 R/C 仍是原椭圆曲线上的数学点，不声称 (ρx,ρy) 仍满足原曲线方程。公开旧 `controlledPointAdd_spec` 保留普通表示，不静默改变它。调用方若在一次编码后连续做多次点加，可直接使用新接口；单次普通表示包装的代价见18.4。实现这项接口扩展须在设计复审中明确确认。
