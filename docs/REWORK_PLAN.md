...
## 26. W5：五位Montgomery窗口（设计，待复审与实现）
...
八项自查：门列与raw/Q记录区别可逐步阅读；只改既有阶段，不建通用窗口框架或运行时开关；README当前值保持；纯文档diff检查；全记录相位及工作清理列为验收条件；T/M与具体物理支持分别计账；最后一窗、control=false、q两种余数与原点加角落不变；新数均待证，明确修正旧收益粗估。收益较小而证明变化较大，建议复审时按−52,544 T/M、+20线这个真实权衡决定是否继续，不能按原−0.3M预期安排实现。
