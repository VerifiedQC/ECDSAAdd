...
依照已复审 REWORK_PLAN §12 的显式门列实现，复用本仓库 Gidney 原地加法器、比较器、常数装卸及 Reduction 数学引理。扩宽约减/比较清借位、低位受控复制和取负组合证明由 Lamport 在本仓库编写，无外部源码复制。正确性与资源针对同一程序，模型仍为带符号基态分支；仅完成四个模加减接口，Horner/适配器尚未实现。
