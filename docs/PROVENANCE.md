...
依照已复审 REWORK_PLAN §12 的显式门列实现，复用本仓库 Gidney 原地加法器、比较器、常数装卸及 Reduction 数学引理。扩宽约减/比较清借位、低位受控复制和取负组合证明由 Lamport 在本仓库编写，无外部源码复制。正确性与资源针对同一程序，模型仍为带符号基态分支；该阶段完成四个模加减接口；后续 C2 见下文，适配器仍待 D。
...
按已复审 §12 实现固定线路上的三 CX 旋转、无控制模加倍/减半及 Horner 正向和清理。复用仓库内 Gidney 加法/比较器、C1 模加减与 HalvingBijection 数学；前缀递推、全部组合证明、精确支持与资源由 Lamport 在本仓库编写，未复制外部源码。C1 的两个阶段引理仅去除 private 供加倍复用。清理用前向模减和减半，模型仍为带符号基态分支。未添加测试、数值 oracle、新公理或放宽证明资源限制；旧域乘法和点加尚未接入该内核。
