...
按REWORK_PLAN §17实现。本项目直接证明基16的整数不变量、secp256k1低位与ZMod转换关系；查表采用16项逐项三层AND匹配、既有受控常数XOR与CZ测量清理。未搬用论文的15-Toffoli查表或更低成本受控加法预算。完整verify实际输出见PROOF_STATUS，P/Q与域乘法接入尚未实现。
