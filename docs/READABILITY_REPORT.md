...
本文是供讨论的整理方案，提交到 `codex/readability-report`，拟由 PR 合入 `new`。本次仅新增报告；下文模块说明、目录调整和协作流程均为建议，尚未实施。报告中的路径相对于仓库根目录，链接相对于本文。
...
建议新增 `docs/MODULES.md` 作为模块地图，模块介绍放在 `docs/modules/`。第一阶段让概念模块对应文件集合，不要求它们立刻拥有独立源码目录。README 提供显眼入口，模块地图说明任务到模块的映射。
...
现有 README 要求文档集中在几份文件中，未实现计划只放 REWORK_PLAN。本报告按用户要求单独提交，供审阅此次文档组织调整；实施模块文档的首个 PR 应同步修订该约定，明确“算法优化计划”和“架构阅读说明”的归属，避免同时保留相互矛盾的规则。
...
`new` 从本报告基线创建，作为整理的集成分支；本报告工作分支为 `codex/readability-report`。后续任务从 `new` 创建独立短期分支，通过以 `new` 为 base 的 PR 集成。本次推送报告，不合并到 `new` 或 `main`。

每个并行任务使用独立 checkout/worktree，避免多个 agent 在同一目录切分支。分支只能隔离修改，不能代替任务边界；任务说明建议包含：
...
| 合并目标 | `new`；遇到跨模块改动先协调责任范围 |

任务交接应简短指出关键结论、源码位置和未解决事项，不要求下一个 agent 重读全部历史对话。不要将“只能读指定文件”设成硬限制；若契约不足，应允许追踪必要源码并补充文档。

沿用仓库现有的合并责任和审阅约定，本报告不重新指定合并人。集成人定期把 `main` 更新合入 `new`，标出受影响的模块说明；工作分支再同步 `new`。共享分支避免改写历史。最终合回 `main` 前，在最终候选提交上进行完整验证并确认文档对应当前实现。
...
- **并行冲突**：每任务独立工作区，明确写入范围；共享布局和导航由集成人协调。
- **整理范围失控**：第一轮以导航和求逆示范验收，再决定后续批次；算法优化继续按既有设计流程处理。

本次交付为这份报告及其 Git 分支。模块介绍、代码重构、PR 合并和完整 Lean 复验均未包含在本次实现中。
