...
该文件判断第 i 步是否仍在计数 K 的有效范围内。计数器为 10 位、i<512，参与线路互异时，`counterActiveXor_spec` 证明：
...
该文件按计数 K 决定是否执行一次模减半或模倍增。布局满足 L.Widths、线路互异，q 为奇数、X<q、2q≤2^L.data.length、i<512。
...
该文件用 512 个固定步骤执行恰好 K 次模减半。沿用 [HalveInPlace.lean](HalveInPlace.lean) 的布局与模数条件，并要求 K≤512。
...
该文件实现求逆核心的准备、恢复及完整异或输出。要求 0<X<q<2^256、q mod 16=15、gcd(q,X)=1；布局使用 512 条记录、10 位计数器、256 位低位输入与算术布局、257 位 a/temp/out，线路互异。
...
该文件对求逆中间值做尺度修正，并保存恢复所需的历史。L.Widths、线路互异，N<q<2^256、q mod 16=15 时，`prepare_spec`、`restore_spec` 证明：
...
该文件提供域求逆接口。L.Widths、线路互异，0<X<p 时，`fieldInverse_xor_spec` 证明：
...
该文件清理或恢复求逆终态中的常量。低位布局为 256 位、q<2^256、线路互异时，`terminalConstants_spec` 和 `terminalConstants_correct` 证明：
...
该文件证明多轮 Kaliski 电路等于数学迭代。令 n=rs.length，要求 i+n≤512、10 位计数器、数据宽度至少为 2、线路互异，初态满足 `KRoundCount i z`、`KInvariant p a z`，且 p、z.u、z.v 能放入低位寄存器。
...
该文件把受控加减结果转移到新寄存器，并清零旧值。src 与布局等宽 n，参与线路互异，令 V=if C then X else 0。
...
该文件将偶数 R 原地变为其模负值，并可恢复。L.Widths n、线路互异，q 为奇数、q<2^n、0<R<2q、R 为偶数时，`negativeEven_spec` 和 `negativeEven_correct` 证明：
...
该文件将输入的模负值异或到目标。设 n=L.width，src、temp、dst 均为 n+1 位，线路互异，0<q<2^n、X<2q。
...
该文件给出模负值初始化的寄存器接口与资源。沿用 [NegativeInit.lean](NegativeInit.lean) 的条件，`negativeInit_spec` 证明：
...
该文件证明一轮 Kaliski 运算及其恢复，只保留一位减法历史。要求 10 位计数器、i<512、线路互异，z 满足 `KRoundCount i z` 和 `KInvariant p a z`，p、z.u、z.v 能放入低位寄存器。此外 p 必须为奇数。
...
该文件记录一轮 Kaliski 的交换和减法分支。线路互异、奇偶临时位与进位工作区为零时，`recordRound_spec` 和 `recordRound_correct` 证明：
...
该文件证明一轮 Kaliski 运算及其恢复，保留交换与减法两位历史。要求 10 位计数器、i<512、线路互异，z 满足 `KRoundCount i z` 和 `KInvariant p a z`，p、z.u、z.v 能放入低位寄存器。
