...
- `L`：单轮求逆布局；active 是本轮使能，r 的最低位用于恢复交换条件，swap 是条件的 XOR 目标。
-/
...
- `L`：Kaliski 单轮布局：u/v 是待约简数据，r/s 是系数，k/kNext 是当前/下一计数寄存器，done 表示终止，active 是本轮使能，swap/subtract 保存分支记录，其余为共享工作位。此接口把 swap 作为轮内临时位，轮间仅保留 subtract。
