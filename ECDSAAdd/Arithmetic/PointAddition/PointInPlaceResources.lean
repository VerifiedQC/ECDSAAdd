...
theorem ControlledPointLayout.inPlaceUsedWires_nodup (L : ControlledPointLayout)
  ...
  have hi := L.inPlaceInverse.usedWires_sublist.count_le q
  simp only [InverseLoopLayout.usedWires,inPlaceUsedWires,List.count_append] at hh hi ⊢
  omega

/-- 6218来自同一门列的支持等式，未重排原9817位分配。 -/
theorem pointInPlaceFinite_qubits (L : ControlledPointLayout) (hw : L.Widths) (hn : L.wires.Nodup)
    (C : Point) (cx cy : Fp) : qubitCount (pointInPlaceFinite L C cx cy)=6218 := by
  ...
  change (L.core.generic::L.point.x++L.point.y++L.inPlaceSlope++L.inPlaceInverse.usedCoreWires).length=6210 at hq
