...
theorem pointCandidate_counts (L : PointAddLayout) (h : L.Widths) (hnd : L.wires.Nodup)
    (cx cy : Fp) :
    (toffoliCount (pointCandidateCompute L cx cy)=4660144 ∧
      measurementCount (pointCandidateCompute L cx cy)=3073200) ∧
    (toffoliCount (pointCandidateClear L cx cy)=4660144 ∧
      measurementCount (pointCandidateClear L cx cy)=3073200) := by
