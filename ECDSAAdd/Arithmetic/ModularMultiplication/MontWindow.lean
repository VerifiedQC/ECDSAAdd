import ECDSAAdd.Arithmetic.ModularMultiplication.MontStagePorts
...
theorem montWindow_correct (L : MontStageLayout) (x y : List Wire) (p i X Y A H : Nat)
    ...
    (run (montWindow L x y p i) m s).phase=s.phase ∧
    (∀ w, w∉L.acc → w∉L.history → (run (montWindow L x y p i) m s).basis w=s.basis w) ∧
    regValue L.acc (run (montWindow L x y p i) m s).basis=montgomeryStep p A X ((Y/16^i)%16) ∧
    regValue L.history (run (montWindow L x y p i) m s).basis=
  ...
  have h1 := montAddDigit_correct L x y i X Y A (by omega) (L.digit_nodup x y hnd) hx hw.pad hw.mask hw.acc hw.carry
    (by omega) hfit s m vx vy va (clean L.pad (by intro w hh; simp [MontStageLayout.work,hh]))
    (clean L.mask (by intro w hh; simp [MontStageLayout.work,hh])) (clean L.carry (by intro w hh; simp [MontStageLayout.work,hh])) (L.cin_clean _ vw)
  let s1 := run (montAddDigit L x y i) m s
  let m1 := m.drop (measurementCount (montAddDigit L x y i))
  have ad := L.acc_disjoint x y hnd
  have keep1 (r : List Wire) (hr : r⊆x++y++L.history++[L.flag]++L.work) : regValue r s1.basis=regValue r s.basis :=
  ...
  have h2 := montReduce_correct L p i U (L.reduce_nodup x y i hnd) (L.record_length hw i hi) hw.scratch
  ...
  have hprog : montWindow L x y p i=montAddDigit L x y i++montReduce L p i := rfl
  ...
  · have hh := mont_replace_value L.history (4*i) 4 hbound (L.history_nodup x y hnd) s.basis t.basis
...
theorem montRestoreWindow_correct (L : MontStageLayout) (x y : List Wire) (p i X Y A H : Nat)
    ...
    (run (montRestoreWindow L x y p i) m s).phase=s.phase ∧
    (∀ w, w∉L.acc → w∉L.history → (run (montRestoreWindow L x y p i) m s).basis w=s.basis w) ∧
    regValue L.acc (run (montRestoreWindow L x y p i) m s).basis=A ∧
    regValue L.history (run (montRestoreWindow L x y p i) m s).basis=H := by
  have hpow : 2^(4*i)=16^i := by rw [Nat.pow_mul]
  have hbound : 4*i+4≤L.history.length := by rw [hw.history]; omega
  have hd : (Y/16^i)%16<16 := Nat.mod_lt _ (by decide)
  have hfit : A+16*X<2^261 := by omega
  ...
  have h1 := montRestoreReduce_correct L p i U (L.reduce_nodup x y i hnd) (L.record_length hw i hi) hw.scratch
  ...
  have work1 : regValue L.work s1.basis=0 := (keep1 _ (by intro w hh; simp [hh])).trans vw
  have clean1 (r : List Wire) (hr : r⊆L.work) : regValue r s1.basis=0 := L.work_clean _ work1 r hr
  have h2 := montSubDigit_correct L x y i X Y A (by omega) (L.digit_nodup x y hnd) hx hw.pad hw.mask hw.acc hw.carry
    (by omega) hfit s1 m1 ((keep1 x (by intro w hh; simp [hh])).trans vx)
    ((keep1 y (by intro w hh; simp [hh])).trans vy) h1.2.2.1
    (clean1 L.pad (by intro w hh; simp [MontStageLayout.work,hh]))
    (clean1 L.mask (by intro w hh; simp [MontStageLayout.work,hh]))
    (clean1 L.carry (by intro w hh; simp [MontStageLayout.work,hh])) (L.cin_clean _ work1)
  rw [montRestoreWindow,run_append,run_take]
  let t := run (montSubDigit L x y i) m1 s1
  have keep (w : Wire) (ha' : w∉L.acc) (hr : w∉L.record i) : t.basis w=s.basis w :=
