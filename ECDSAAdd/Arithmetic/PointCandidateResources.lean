...
theorem pointCandidate_counts (L : PointAddLayout) (h : L.Widths) (hnd : L.wires.Nodup)
    (cx cy : Fp) :
    (toffoliCount (pointCandidateCompute L cx cy)=5161384 ∧
      measurementCount (pointCandidateCompute L cx cy)=3310248) ∧
    (toffoliCount (pointCandidateClear L cx cy)=5161384 ∧
      measurementCount (pointCandidateClear L cx cy)=3310248) := by
