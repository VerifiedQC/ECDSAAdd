...
theorem pointCandidate_counts (L : PointAddLayout) (h : L.Widths) (hnd : L.wires.Nodup)
    (cx cy : Fp) :
    (toffoliCount (pointCandidateCompute L cx cy)=22989640 ∧
      measurementCount (pointCandidateCompute L cx cy)=13258824) ∧
    (toffoliCount (pointCandidateClear L cx cy)=22989640 ∧
      measurementCount (pointCandidateClear L cx cy)=13258824) := by
