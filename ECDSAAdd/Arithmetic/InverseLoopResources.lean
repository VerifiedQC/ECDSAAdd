...
theorem inverseLoop_resources (L : InverseLoopLayout) (hnd : L.wires.Nodup)
    ...
    toffoliCount (inverseLoop L q)=1024*(21*L.first.data.width+83)+60*L.first.data.width-12 ∧
    measurementCount (inverseLoop L q)=1024*(8*L.first.data.width+79)+48*L.first.data.width ∧
  ...
  · simp only [inverseLoop,inverseCompute,inverseUncompute,measurementCount_append,
    ...
    simp only [InverseLoopLayout.halving]
    omega
  · rw [qubitCount,inverseLoop_wires L hn hw hd hwidth ha ht hout q,List.toFinset_card_of_nodup hnd]
    ...
    have hl : L.arithmetic.wires.length=8*(L.arithmetic.width+1)+2 := by
      ...
      omega
    simp only [InverseLoopLayout.wires,InverseLoopLayout.extra,List.length_append,ha,ht,hout,hl]
    omega
...
theorem inverseLoop_257_resources (L : InverseLoopLayout) (hnd : L.wires.Nodup)
    ...
    toffoliCount (inverseLoop L q)=5626928 ∧ measurementCount (inverseLoop L q)=2198576 ∧
