...
- 规格：偶数 X 右循环移位一位得到 X/2；高位为零且位宽满足前提时，左循环移位一位得到 2X。
- 正确性：这里的 `_correct` 证明基本 swapBits 确实交换两位，并保持其余基态位和相位；移位数值结论由 `_spec` 给出。
...
- 规格：受控交换只在 C 为真时交换两位；满足边界位条件时，受控右/左移位只在 C 为真时得到 X/2 或 2X，否则保持 X，控制不变。
- 正确性：这里的 `_correct` 证明 cswap 在控制开启时交换两位，否则保持它们，其他所有基态位与相位不变。
- 资源：
