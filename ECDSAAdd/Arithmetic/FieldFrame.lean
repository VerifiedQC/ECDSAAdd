...
theorem fieldMul_correct (L : MulAdapterLayout) (hnd : L.wires.Nodup) (hw : L.Widths)
  ...
  intro w ho
  exact mulXor_frame L p _ _ _ hw hnd (by norm_num [p]) (by rw [hn]; norm_num [p]) hx
    (by simpa only [show L.y.length=L.width from hw.1.y] using regValue_lt L.y s.basis)
