...
theorem pointInPlaceProduct_correct (L : ControlledPointLayout) (hw : L.Widths)
    ...
    ((run (mulAdd L.inPlaceMultiply p) m s).phase=s.phase ∧
      regValue L.point.y (run (mulAdd L.inPlaceMultiply p) m s).basis=(Y+(A*X)%p)%p ∧
      ∀ q∉L.point.y,(run (mulAdd L.inPlaceMultiply p) m s).basis q=s.basis q) ∧
    ((run (mulSub L.inPlaceMultiply p) m s).phase=s.phase ∧
      regValue L.point.y (run (mulSub L.inPlaceMultiply p) m s).basis=(Y+p-(A*X)%p)%p ∧
      ∀ q∉L.point.y,(run (mulSub L.inPlaceMultiply p) m s).basis q=s.basis q) := by
  ...
  have hout : regValue L.inPlaceMultiply.out s.basis=Y := by
    ...
    simp [regValue,h1]
  have hwidth : L.inPlaceMultiply.width=256 := by
    simp only [MulAdapterLayout.width,inPlaceMultiply,inPlaceUnary,List.length_take,List.length_drop,
      L.inPlaceBorrow_length hw]
    norm_num
  have hpn : p<2^L.inPlaceMultiply.width := by rw [hwidth]; exact hp
  have hX' : X<2^L.inPlaceMultiply.width := hX.trans hpn
  have hwf := L.inPlaceMultiply_widths hw
  have hnf := L.inPlaceMultiply_nodup hw hnd
  have keep := mulAddSub_frame L.inPlaceMultiply p A X Y hwf hnf (by norm_num [p]) hpn hA hX' hY
  ...
  have finish (P : Program) (V : Nat) (hV : V<p)
    ...
    · apply hframe q
      ...
      simp [hq,he]
  obtain ⟨hpa,hva⟩ := mulAdd_spec L.inPlaceMultiply p A X Y hwf hnf (by norm_num [p]) hpn hA hX' hY
    s m ⟨⟨⟨hin,hx⟩,hout⟩,hwork⟩
  obtain ⟨hps,hvs⟩ := mulSub_spec L.inPlaceMultiply p A X Y hwf hnf (by norm_num [p]) hpn hA hX' hY
    s m ⟨⟨⟨hin,hx⟩,hout⟩,hwork⟩
